/*
 * uart_rx.v
 *
 * Standard UART Receiver (8 data bits, 1 stop bit, no parity).
 *
 * This module uses a 16x oversampling technique to sample the incoming 
 * serial data at the center of each bit, ensuring reliable data recovery 
 * even with minor clock frequency mismatches.
 */
`default_nettype none
module uart_rx #(
    parameter CLK_FREQ = 50_000_000, 
    parameter BAUDRATE = 115200      
)(
    input  wire clk_i,      
    input  wire reset_i,    
    input  wire rx_i,       

    output reg  [7:0] data_o,  
    output reg        valid_o, 
    input  wire       ready_i  
);

    localparam integer CLKS_PER_TICK = CLK_FREQ / (BAUDRATE * 16); // 27 para 50MHz

    // OPTIMIZACIÓN: Para contar hasta 27, solo necesitamos 5 bits (2^5 = 32).
    reg [4:0] clk_cnt;
    reg       tick;

    always @(posedge clk_i or posedge reset_i) begin
        if (reset_i) begin
            clk_cnt <= 5'd0;
            tick    <= 1'b0;
        end else begin
            if (clk_cnt == CLKS_PER_TICK - 1) begin
                clk_cnt <= 5'd0;
                tick    <= 1'b1;
            end else begin
                clk_cnt <= clk_cnt + 5'd1;
                tick    <= 1'b0;
            end
        end
    end

    reg [1:0] sync;
    always @(posedge clk_i) begin
        sync <= {sync[0], rx_i};
    end
    wire rx = sync[1];

    localparam IDLE = 3'd0, START = 3'd1, DATA = 3'd2, STOP = 3'd3, WAIT = 3'd4;

    reg [2:0] state;
    reg [3:0] tick_cnt; 
    reg [2:0] bit_cnt;  
    reg [7:0] shift;    

    always @(posedge clk_i or posedge reset_i) begin
        if (reset_i) begin
            state    <= IDLE;
            tick_cnt <= 4'd0;
            bit_cnt  <= 3'd0;
            shift    <= 8'd0;
            data_o   <= 8'd0;
            valid_o  <= 1'b0;
        end else begin
            case (state)
                IDLE: begin
                    valid_o <= 1'b0;
                    if (rx == 1'b0) begin
                        state    <= START;
                        tick_cnt <= 4'd0;
                    end
                end
                START: if (tick) begin
                    if (tick_cnt == 4'd7) begin
                        if (rx == 1'b0) tick_cnt <= tick_cnt + 4'd1;
                        else state <= IDLE; 
                    end else if (tick_cnt == 4'd15) begin
                        tick_cnt <= 4'd0;
                        bit_cnt  <= 3'd0;
                        state    <= DATA;
                    end else begin
                        tick_cnt <= tick_cnt + 4'd1;
                    end
                end
                DATA: if (tick) begin
                    if (tick_cnt == 4'd7) begin 
                        shift[bit_cnt] <= rx;
                        tick_cnt <= tick_cnt + 4'd1;
                    end else if (tick_cnt == 4'd15) begin
                        tick_cnt <= 4'd0;
                        if (bit_cnt == 3'd7) state <= STOP;
                        else bit_cnt <= bit_cnt + 3'd1;
                    end else begin
                        tick_cnt <= tick_cnt + 4'd1;
                    end
                end
                STOP: if (tick) begin
                    if (tick_cnt == 4'd15) begin
                        data_o  <= shift;
                        valid_o <= 1'b1;
                        state   <= WAIT;
                        tick_cnt <= 4'd0;
                    end else begin
                        tick_cnt <= tick_cnt + 4'd1;
                    end
                end
                WAIT: begin
                    if (ready_i) begin
                        valid_o <= 1'b0;
                        state   <= IDLE;
                    end
                end
                default: state <= IDLE;
            endcase
        end
    end
endmodule